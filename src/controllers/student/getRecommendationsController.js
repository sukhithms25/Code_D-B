...
module.exports = catchAsync(async (req, res, next) => {
  const { topic = 'programming', difficulty = 'beginner' } = req.query;
  const resources = await resourceRecommenderService.recommendResources(topic, difficulty);
  res.status(200).json(new ApiResponse(200, resources, 'Resource recommendations retrieved successfully'));
});
